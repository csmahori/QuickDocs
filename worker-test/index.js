/**
 * EasyDocuments — Razorpay TEST Payment Worker
 * Cloudflare Worker: handles order creation + signature verification (TEST MODE)
 *
 ...
 * Changes require redeploy: cd worker-test && npx wrangler deploy
 * ⚠️ Keep this table in sync with worker/index.js (prod).
 */
const COUPONS = {
  'SAVE50':  { pct: 50,  active: true },
  'HARDAPK75': { pct: 75,  active: true },
  'PAHADI100': { pct: 100, active: true },
};
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/create-order') {
        const body = await request.json();
        let amount = Number(body.amount) || 900;
